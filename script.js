...
function animateCounter(element, target) {
    ...
    const increment = target / 100;
     const isCoffeeCounter = element.nextElementSibling?.textContent.includes('Coffee');
    const timer = setInterval(() => {
        ...
        element.textContent = Math.floor(current) + (isCoffeeCounter? '+': '');
    }, 20);
}
...
function typeText(element, text, speed = 100, delay = 1000) {
    let index = 0;
...
window.addEventListener('load', () => {
    ...
    typeText(nameElement, 'Jaymark Tabiolo E', 150);
});
